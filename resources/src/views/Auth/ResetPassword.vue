      ...
      <div class="relative flex flex-col justify-center w-full h-screen lg:flex-row dark:bg-gray-900">
        <div class="flex flex-col flex-1 w-full lg:w-1/2">
          ...
          <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
            ...
            <div v-if="error" class="mb-4 p-3 text-sm text-red-600 bg-red-50 rounded-lg dark:bg-red-900/20 dark:text-red-400">
              {{ error }}
            </div>

            <div v-if="success" class="mb-4 p-3 text-sm text-green-600 bg-green-50 rounded-lg dark:bg-green-900/20 dark:text-green-400">
              {{ success }}
            </div>
            ...
            <form @submit.prevent="handleSubmit" class="space-y-5">
              ...
              <div>
                <label for="password" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  {{ t('auth.reset_password.fields.password.label') }}<span class="text-error-500">*</span>
                </label>
                <div class="relative">
                  <input
                    v-model="password"
              ...
              <div>
                <label for="confirmPassword" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  {{ t('auth.reset_password.fields.password_confirmation.label') }}<span class="text-error-500">*</span>
                </label>
                <div class="relative">
                  <input
              ...
              >
                <span v-if="isLoading" class="flex items-center">
                  ...
                  {{ t('auth.reset_password.submit_button_loading') }}
                </span>
                <span v-else>
...
const handleSubmit = async () => {
  ...
  if (!token || !email) {
    error.value = t('auth.reset_password.error.invalid_link')
    return
  }

  if (password.value !== confirmPassword.value) {
    error.value = t('auth.reset_password.error.password_mismatch')
    return
  }

  if (password.value.length < 8) {
    error.value = t('auth.reset_password.error.password_too_short')
    return
  }
  ...
  try {
    await authStore.resetPassword(token, email, password.value)
    success.value = t('auth.reset_password.success')
    // Redirige après succès
    setTimeout(() => {
      router.push({ path: '/signin' })
    }, 1500)
  } catch (err: any) {
    error.value = err.message || t('auth.reset_password.error.unexpected')
  } finally {
